...
def face_detect(images):
	...
	results = []
	pady1, pady2, padx1, padx2 = hp.pads
	...
	if not hp.nosmooth: boxes = get_smoothened_boxes(boxes, T=5)
...
def datagen(frames, mels, static):
	...
	for i, m in enumerate(mels):
		...
		if len(img_batch) >= hp.wav2lip_batch_size:
...
def generate(face_path, input_audio, weights, rotate = False):
	...
	if not path.isfile(face_path):
		...
		while 1:
			...
			if not still_reading:
				...
				break
			if hp.resize_factor > 1:
				frame = cv2.resize(frame, (frame.shape[1]//hp.resize_factor, frame.shape[0]//hp.resize_factor))
	...
	batch_size = hp.wav2lip_batch_size
